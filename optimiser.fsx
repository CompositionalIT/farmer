...
let optimise containers (cores:float<VCores>) (memory:float<Gb>) =
    ...
    let minCores = containers * 0.25<VCores>
    let minRam = containers * 0.5<Gb>
    if minCores > cores then Error $"Insufficient cores (minimum is {minCores}VCores)."
        ...
        let vcoresPerContainer = Math.Round ((cores / containers) * 4., MidpointRounding.ToZero) / 4.
        let remainingCores = cores - (vcoresPerContainer * containers)

        let gbPerContainer = Math.Round ((memory / containers) * 2., MidpointRounding.ToZero) / 2.
        let remainingGb = memory - (gbPerContainer * containers)

        Ok
            [
                for c in 1. .. containers do
                    if c = 1. then (vcoresPerContainer + remainingCores) * 1.<VCores>, (gbPerContainer + remainingGb) * 1.<Gb>
...
type Tests =
    static member totalsAlwaysEqualInput (ValidInput(PositiveInt containers, cores, memory)) =
        ...
        match split with
        | Ok split ->
            let correctCores = split |> List.sumBy fst = cores
            let correctRam = split |> List.sumBy snd = memory
    ...
    static member neverReturnsLessThanMinimum (ValidInput(PositiveInt containers, cores, memory)) =
        let split = optimise containers cores memory
        match split with
        | Ok split -> split |> List.forall(fun (c, m) -> c >= 0.25<VCores> && m >= 0.5<Gb>)
        | Error msg -> failwith msg
...
let basicGen = gen {
    let! cores, gb = Gen.elements data
...
let shrinker checker (con:PositiveInt, cor, mem) =
    [
        ...
        if cor > 0.25<VCores> then con, cor - 0.25<VCores>, mem
        if mem > 0.5<Gb> then con, cor, mem - 0.5<Gb>
    ]
    |> List.filter checker

type ResourceArb =
    static member IsValid (PositiveInt con, cor, mem) =
        let cores = float con * 0.25 <= float cor
        let memory = float con * 0.5 <= float mem
        cores && memory
    ...
    static member ValidInputs () =
        { new Arbitrary<ValidInput> () with
            override _.Generator =
                basicGen
                |> Gen.filter ResourceArb.IsValid
                |> Gen.map ValidInput
            override _.Shrinker (ValidInput inputs) =
                inputs
                |> shrinker ResourceArb.IsValid
                |> Seq.map ValidInput
        }
    static member InvalidInputs () =
        { new Arbitrary<InvalidInput> () with
            override _.Generator =
                basicGen
                |> Gen.filter (ResourceArb.IsValid >> not)
                |> Gen.map InvalidInput
            override _.Shrinker (InvalidInput x) =
                x
                |> shrinker (ResourceArb.IsValid >> not)
                |> Seq.map InvalidInput
        }
