    ...
    testList
        ...
        [
            ...
            test "Can create a basic dedicated host group with a host" {
                ...
                let parentResourceId = Arm.Compute.hostGroups.resourceId (ResourceName parentHostGroupName)
                Expect.hasLength dependsOn 1 "Should only depend on one resource, the host group"
                Expect.contains dependsOn (string parentResourceId) $"Parent host group is incorrect, should be {parentHostGroupName}"
                ()
            }
            test "Can create a host group with a few availability zones and a valid domain count" {
                ...
                Expect.contains zones (AvailabilityZone.ToArmValue AvailabilityZone.One) "The zones should contain zone 1"
                Expect.contains zones (AvailabilityZone.ToArmValue AvailabilityZone.Two) "The zones should contain zone 2"
                ()
            }
        ]