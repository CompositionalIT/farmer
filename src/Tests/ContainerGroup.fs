    ...
    testList
        ...
        [ test "Single container in a group is correctly created" {
          ...
          test "Multiple containers are correctly added" {
              let group =
                  containerGroup { add_instances [ nginx; fsharpApp ] }
                  |> asAzureResource
          ...
          test "Implicitly creates public ports for group based on instances" {
              let group =
                  containerGroup { add_instances [ nginx ] }
                  |> asAzureResource

              let ports =
                  group.IpAddress.Ports
                  |> Seq.map (fun p -> p.PortProperty)
                  |> Seq.toList
...
"""
                  ...
                  containerGroup {
                      ...
                      add_instances
                          [ containerInstance {
                                ...
                                command_line (
                                    "dotnet fsi /app/src/main.fsx".Split null
                                    |> List.ofArray
                                )
          ...
          test "Container groups with subnetIds and netprofile uses correct API versions" {
              ...
              let netProfileApiVersion =
                  containerGroupNetProfile.["apiVersion"]
                  |> string
                  |> DateOnly.Parse
              ...
              let subnetIdsApiVersion =
                  containerGroupSubnetIds.["apiVersion"]
                  |> string
                  |> DateOnly.Parse
          ...
          test "Container network profile with vnet has expected dependsOn" {
              ...
              let actualContainerNetDeps =
                  (dependsOn :?> Newtonsoft.Json.Linq.JArray)
                      .First.ToString()
