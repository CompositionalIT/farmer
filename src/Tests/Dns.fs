...
let tests = testList "DNS Zone" [
    test "Public DNS Zone is created with a CNAME record" {
        ...
        Expect.equal dnsRecords.[3].SoaRecord.MinimumTtl (Nullable<int64>(300L)) "DNS SOA record minimum ttl wrong"
        Expect.equal dnsRecords.[3].SoaRecord.RefreshTime (Nullable<int64>(3600L)) "DNS SOA record refresh time wrong"
        Expect.equal dnsRecords.[3].SoaRecord.RetryTime (Nullable<int64>(300L)) "DNS SOA record retry time wrong"
        Expect.equal dnsRecords.[3].SoaRecord.ExpireTime (Nullable<int64>(2419200L)) "DNS SOA record expire time wrong"
