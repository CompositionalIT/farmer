...
type FirewallResource =
    { name: string
      ...
      ``type``: string
      dependsOn: string array
      properties: {| endIpAddress: string
        ...
        [ test "Server settings are correct" {
          ...
          test "Vnet rules are correctly set" {
              ...
              let actual =
                  postgreSQL {
                      ...
                      add_vnet_rules
                          [ vnetRuleName1, networkResourceId1
                            vnetRuleName2, networkResourceId2 ]
                  }
          ...
          test "Server name must be given" {
              Expect.throws
                  (fun () ->
                      runBuilder
                      <| postgreSQL { admin_username "adminuser" }
                      |> ignore)
                  "Missing server name"
          }
          ...
          test "Admin username must be given" {
              Expect.throws
                  (fun () ->
                      runBuilder <| postgreSQL { name "servername" }
                      |> ignore)
