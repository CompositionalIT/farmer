...
let client = new OperationalInsightsManagementClient (Uri "http://management.azure.com", TokenCredentials "NotNullOrWhiteSpace")
let logicAppResource = ResourceType("Microsoft.Logic/workflows", "").resourceId "LogicApp"
let getDiagnosticsResource t = findAzureResourcesByType<DiagnosticSettingsResource> (Arm.DiagnosticSetting.diagnosticSettingsType t) client.SerializationSettings >> List.head
...
let tests = testList "Diagnostic Settings" [
    test "Creates diagnostic settings with raw sinks" {
        ...
        let result = getDiagnosticsResource logicAppResource.Type template
