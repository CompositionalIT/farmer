    ...
    testList
        ...
        [ test "Can create secrets without popping" { secret { name "test" } |> ignore }
          ...
          test "Creates key vault secrets correctly" {
              ...
              Expect.throws
                  (fun _ ->
                      SecretConfig.create ("bad", (ArmExpression.literal "foo"))
                      |> ignore)
          ...
          test "Create a basic key vault" {
              ...
              Expect.equal
                  kvName
                  (JValue.CreateString "my-test-kv-9876abcd" :> JToken)
                  "Incorrect name set on key vault"
          }
          ...
          test "Add access policies on existing key vault" {
              ...
              let tenant =
                  jobj.SelectToken("resources[0].properties.accessPolicies[0].tenantId")
                  |> string
