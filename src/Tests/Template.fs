...
let tests =
    testList
        ...
        [
            ...
            test "Can create multiple resources simultaneously" {
                let template =
                    arm { add_resources [ storageAccount { name "test" }; storageAccount { name "test2" } ] }
            ...
            test "De-dupes the same resource name and type" {
                let template =
                    arm { add_resources [ storageAccount { name "test" }; storageAccount { name "test" } ] }
            ...
            test "Does not de-dupe the same resource name but different type" {
                let template =
                    arm { add_resources [ storageAccount { name "test" }; cognitiveServices { name "test" } ] }
            ...
            test "Can add a list of resources types together" {
                let resources: IBuilder list =
                    [ storageAccount { name "test" }; storageAccount { name "test2" } ]
  ...
  "secretName": "vm-password"
                ...
                Expect.equal
                    parametersJson
                    "{}"
                    "Outer template should not have parameter that is passed to inner template"
            }
            test "Can reference vault secret from another resource group" {
                ...
                let resourceGroupBeingDeployed =
                    resourceGroup {
