    ...
    testList
        ...
        [
            ...
            test "Supports slot settings" {
                ...
                let scn =
                    functionsApp |> getResources |> getResource<Web.SlotConfigName> |> List.head
                ...
                let appSettingNames =
                    jobj.SelectTokens
                        $"$..resources[?(@name=='{functionsApp.Name.ResourceName.Value}/slotconfignames')].properties.appSettingNames[*]"
                    |> Seq.map (fun x -> x.ToString())

                let dependencies =
                    jobj.SelectTokens
                        $"$..resources[?(@name=='{functionsApp.Name.ResourceName.Value}/slotconfignames')].dependsOn[*]"
                    |> Seq.map (fun x -> x.ToString())
                ...
                Expect.containsAll
                    dependencies
                    [
                        $"[resourceId('Microsoft.Web/sites', '{functionsApp.Name.ResourceName.Value}')]"
                    ]
