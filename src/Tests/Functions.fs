    ...
    testList
        ...
        [ test "Renames storage account correctly" {
          ...
          test "Implicitly sets dependency on connection string" {
              ...
              let site =
                  f.BuildResources Location.NorthEurope
                  |> List.item 3
                  :?> Web.Site
          ...
          test "Works with unmanaged storage account" {
              ...
              Expect.isFalse
                  (resources
                   |> List.exists (fun r -> r.ResourceId.Type = storageAccounts))
                  "Storage Account should not exist"

              Expect.isFalse
                  (site.Dependencies
                   |> Set.contains externalStorageAccount)
          ...
          test "Handles identity correctly" {
              ...
              Expect.sequenceEqual
                  (f.Identity.UserAssignedIdentities
                   |> Seq.map (fun r -> r.Key))
          ...
          test "Service plans support Elastic Premium functions" {
              ...
              let resources =
                  (sp :> IBuilder)
                      .BuildResources Location.WestEurope
