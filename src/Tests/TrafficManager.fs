...
let dummyClient = new TrafficManagerManagementClient(Uri "http://management.azure.com", TokenCredentials "NotNullOrWhiteSpace")

let tests = testList "Traffic Manager" [
    ...
    test "Can create a basic default Traffic Manager profile with sensible defaults" {
        ...
        Expect.equal resource.TrafficViewEnrollmentStatus (Nullable Models.TrafficViewEnrollmentStatus.Disabled) "Default TrafficViewEnrollmentStatus is incorrect"
        ...
        Expect.equal resource.MonitorConfig.IntervalInSeconds (Nullable (int64 30)) "Default MonitorConfig IntervalInSeconds is incorrect"
