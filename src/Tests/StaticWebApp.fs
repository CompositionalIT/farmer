...
let tests =
    testList "Static Web App Tests" [
        test "Creates a basic static web app" {
            ...
            let swaArm =
                (swa :> IBuilder).BuildResources(Location.WestEurope).[0] :?> StaticSite
        ...
        test "Supports app settings" {
            let swa = staticWebApp {
                name "foo"
                repository "https://compositional-it.com"
                app_settings [ "foo", "bar"; "blip", "blop" ]

            }

            let swaArm =
                (swa :> IBuilder).BuildResources(Location.WestEurope).[1] :?> StaticSites.Config
