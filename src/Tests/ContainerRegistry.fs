...
type RegistryJson =
    { resources: {| name: string
                    ...
                    sku: {| name: string |}
                    location: string
                    properties: IDictionary<string, System.Text.Json.JsonElement> |} array }
...
let shouldHaveAdminUserEnabled (r: RegistryJson) =
    let b =
        resource(r).properties.["adminUserEnabled"]
            .GetBoolean()
...
let tests =
    testList
        ...
        [ test "Basic resource settings are written to template resource" {
          ...
          testList
              "Container Registry Name Validation tests"
              [ let invalidNameCases =
                    [ "Empty Account", "", "cannot be empty", "Name too short"
                      "Min Length",
                      "abc",
                      "min length is 5, but here is 3. The invalid value is 'abc'",
                      "Name too short"
                      "Max Length",
