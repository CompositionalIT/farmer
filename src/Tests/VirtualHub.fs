...
let hubRouteTableAsAzureResource (routeTable: HubRouteTableConfig) =
    ...
    |> findAzureResources<Microsoft.Azure.Management.Network.Models.VirtualHubRouteTableV2>
...
let virtualHubTests =
    testList
        "VirtualHub only Tests"
        [
            test "VirtualHub is correctly created" {
                let vhub = vhub { name "my-vhub" } |> asAzureResource
            ...
            test "VirtualHub creates empty dependsOn in arm template json for unmanaged linked resources" {
                ...
                let dependsOn =
                    getResourceDependsOnByName template.Deployment (ResourceName "my-vhub")
...
let hubRouteTableTests =
    testList
        "Hub Route Table Tests"
        [
                            ...
                            [
                                {
                                    ...
                                    NextHop =
                                        NextHop.ResourceId(LinkedResource.Unmanaged(virtualHubs.resourceId "next-hub"))
                                }
                            ]
                    }
                    |> hubRouteTableAsAzureResource
            ...
            test "HubRouteTable creates empty dependsOn in arm template json for unmanaged linked resources" {
                ...
                let dependsOn =
                    getResourceDependsOnByName template.Deployment (vhubResourceName / routeTableResourceName)
            ...
            test "HubRouteTable creates dependsOn in arm template json for managed linked resources" {
                ...
                let expectedDependency =
                    $"[resourceId('Microsoft.Network/virtualHubs', '{vhubResourceName.Value}')]"
