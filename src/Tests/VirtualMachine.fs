    ...
    testList "Virtual Machine" [
        test "Can create a basic virtual machine" {
            ...
            Expect.equal
                ...
                "[reference(resourceId('Microsoft.Storage/storageAccounts', 'isaacsvmstorage'), '2025-06-01').primaryEndpoints.blob]"
            ...
            let deployment = arm {
                add_resources [
                    vm {
                        ...
                        diagnostics_support_external (Farmer.Arm.Storage.storageAccounts.resourceId "vmdiagstorage")
                    }
                ]
            }
                ...
                add_resources [
                    vm {
                        ...
                        diagnostics_support_external (
                            LinkedResource.Managed(Farmer.Arm.Storage.storageAccounts.resourceId "vmdiagstorage")
                        )
                    }
                ]
            }
            ...
            let deployment = arm {
                add_resources [
                    vm {
                        ...
                        private_ip_allocation (PrivateIpAddress.StaticPrivateIp(Net.IPAddress((int64 0x2414188f))))
                    }
                ]
            }
            ...
            let deployment = arm {
                add_resources [
                    vm {
                        ...
                        private_ip_allocation (PrivateIpAddress.StaticPrivateIp(Net.IPAddress.Parse("192.168.12.13")))

                        add_ip_configurations [
                            ipConfig {
                                private_ip_allocation (
                                    PrivateIpAddress.StaticPrivateIp(Net.IPAddress.Parse("192.168.12.14"))
                                )
                            }
                        ]
                    }
        ...
        test "Creates VM and attaches newly imported OS disk" {
            ...
            Expect.equal vmOsDisk.["name"] (JValue "imported-disk-image") "OS disk name should match attached disk name"
