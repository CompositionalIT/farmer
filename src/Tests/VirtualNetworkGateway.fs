...
let tests =
    testList "VirtualNetworkGateway" [
        test "Can create a basic VirtualNetworkGateway" {
            ...
            let gw =
                resources
                |> List.pick (function
                    | :? VirtualNetworkGateway as v -> Some v
                    | _ -> None)
        ...
        test "Can create a VirtualNetworkGateway with VpnClientConfiguration" {
            ...
            Expect.equal
                gw.VpnClientConfiguration.Value.ClientAddressPools
                [
                    {
                        Address = Net.IPAddress.Parse "10.31.0.0"
                        Prefix = 16
                    }
                    {
                        Address = Net.IPAddress.Parse "10.32.1.0"
                        Prefix = 24
                    }
                ]
...
-----END CERTIFICATE-----"""
            ...
            Expect.equal
                ...
                [
                    ...
                    {|
                        Name = "root2"
                        PublicCertData = "IQfNUTod7Jl7ZOacFlV3fvJTANBgkqhTER7A0qo591ewpAPMpugHh9eQ3ucR5o"
                    |}
                ]
        ...
        test "Can create a VirtualNetworkGateway with protocols" {
            let b =
                gateway {
                    name "gateway"
                    vpn_client (vpnclient { protocols [ OpenVPN; IkeV2 ] })
                }
                :> IBuilder
        ...
        test "Can create a VirtualNetworkGateway with default protocol" {
            let b =
                gateway {
                    name "gateway"
                    vpn_client (vpnclient { add_address_pool "10.31.0.0/16" })
                }
                :> IBuilder
