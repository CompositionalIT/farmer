    ...
    testList
        ...
        [ test "Can add two identities together" {
          ...
          test "Creates ARM JSON correctly" {
              ...
              let testIdentity =
                  userAssignedIdentities.resourceId "test"
              ...
              let json =
                  let testIdentity2 =
                      userAssignedIdentities.resourceId "test2"
                      |> ManagedIdentity.create

                  { ManagedIdentity.Empty with SystemAssigned = Enabled }
                  + testIdentity
                  + testIdentity2
                  + testIdentity2
                  |> ManagedIdentity.toArmJson
