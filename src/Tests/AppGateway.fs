...
let tests =
    testList "Application Gateway Tests" [
        test "Empty basic app gateway" {
            let ag = appGateway { name "ag" }
            ()
        ...
        test "Complex App gateway" {
            ...
            Expect.equal
                backendPool.BackendAddresses.[0].IpAddress
                "10.28.1.4"
                "Backend address pool has incorrect IP in pool - item 1"

            Expect.equal
                backendPool.BackendAddresses.[1].IpAddress
                "10.28.1.5"
                "Backend address pool has incorrect IP in pool - item 2"
            ...
            Expect.equal
                gwipconf.Subnet.Id
                "[resourceId('Microsoft.Network/virtualNetworks/subnets', 'agw-vnet', 'gw')]"
                "Gateway IP subnet ID did not match"
            ...
            Expect.equal
                ...
                "[resourceId('Microsoft.Network/applicationGateways/frontendPorts', 'app-gw', 'port-80')]"
                "Listener port did not match"

            Expect.equal
                httpListener.FrontendIPConfiguration.Id
                "[resourceId('Microsoft.Network/applicationGateways/frontendIPConfigurations', 'app-gw', 'app-gw-fe-ip')]"
                "Listener ipconfig did not match"
            ...
            Expect.equal
                ...
                "[resourceId('Microsoft.Network/applicationGateways/httpListeners', 'app-gw', 'http-listener')]"
                "Routing rule listener id did not match"

            Expect.equal
                routingRule.BackendAddressPool.Id
                "[resourceId('Microsoft.Network/applicationGateways/backendAddressPools', 'app-gw', 'agw-be-pool')]"
                "Routing rule pool did not match"

            Expect.equal
                routingRule.BackendHttpSettings.Id
