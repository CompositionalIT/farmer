...
type WorkspaceJson =
    { name: string
      ...
      sku: {| name: string |}
      properties: {| managedResourceGroupId: string
                     parameters: {| enableNoPublicIp: bool ValueObj
                                    ...
                                    encryption: {| keySource: string
                                                   keyName: string
                                                   keyversion: string
                                                   keyvaulturi: string |} ValueObj |} |} }

let fromJson (db: DatabricksConfig) =
    toTypedTemplate<WorkspaceJson> Location.NorthEurope db

let tests =
    testList
        "Databricks Tests"
        [ let getWorkspaceArm (db: DatabricksConfig) =
              (db :> IBuilder)
                  .BuildResources Location.NorthEurope
              |> List.head
              :?> Workspace
          ...
          test "Creates a basic workspace" {
              ...
              Expect.equal
                  bricks.properties.managedResourceGroupId
                  "[concat(subscription().id, '/resourceGroups/', 'databricks-workspace-rg')]"
                  "Wrong managed resource group name"
          ...
          test "Allows overriding managed resource group name" {
              let bricks =
                  databricks { managed_resource_group_id "databricks-rg" }
                  |> fromJson

              Expect.equal
                  bricks.properties.managedResourceGroupId
                  "[concat(subscription().id, '/resourceGroups/', 'databricks-rg')]"
                  "Wrong managed resource group name"
          }

          test "Handles use_public_ip feature flag" {
              let bricks =
                  databricks { allow_public_ip Disabled }
                  |> fromJson
          ...
          test "Sets BYOV configuration" {
              let bricks =
                  databricks { attach_to_vnet "databricks-vnet" "databricks-pub-snet" "databricks-priv-snet" }
                  |> fromJson

              Expect.equal
                  bricks.properties.parameters.customVirtualNetworkId.value
                  "[resourceId('Microsoft.Network/virtualNetworks', 'databricks-vnet')]"
                  "BYOV vnet is not set correctly"

              Expect.equal
                  bricks.properties.parameters.customPublicSubnetName.value
                  "databricks-pub-snet"
                  "BYOV public subnet not set correctly"

              Expect.equal
                  bricks.properties.parameters.customPrivateSubnetName.value
                  "databricks-priv-snet"
                  "BYOV private subnet not set correctly"
              ...
              let bricks =
                  databricks { attach_to_vnet vn pubSubnet privSubnet }
                  |> fromJson
          ...
          test "Encryption works correctly" {
              ...
              Expect.equal
                  parameters.encryption.value.keyName
                  "databricks-encryption-key"
                  "Key name not initialised correctly"

              Expect.isNull parameters.encryption.value.keyversion "Key version not initialised correctly"

              Expect.equal
                  parameters.encryption.value.keyvaulturi
                  "https://databricks-kv.vault.azure.net"
                  "Key vault uri not initialised correctly"
              ...
              Expect.equal
                  bricks.properties.parameters.encryption.value.keyversion
                  "74135499-7a08-45fa-9ebd-94670097b04a"
                  "Key vault version not set correctly"

              Expect.throws
                  (fun () ->
                      databricks { key_vault_key_version Guid.Empty }
                      |> ignore)
                  "Should not be able to set key version without vault config"

              let bricks =
                  databricks {
                      sku Premium
                      encrypt_with_databricks
                  }
                  |> fromJson

              Expect.equal
                  bricks.properties.parameters.encryption.value.keySource
                  "Default"
                  "encryption mode should be databricks"

              Expect.throws
                  (fun () ->
                      databricks { encrypt_with_key_vault "test" "test" }
                      |> ignore)
                  "Should not be able to set key vault without Premium set"
          } ]
