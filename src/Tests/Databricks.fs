    ...
    testList
        ...
        [
            let getWorkspaceArm (db: DatabricksConfig) =
                (db :> IBuilder).BuildResources Location.NorthEurope |> List.head :?> Workspace

            test "Creates a basic workspace" {
                let bricks = databricks { name "databricks-workspace" } |> fromJson
            ...
            test "Encryption works correctly" {
                ...
                Expect.throws
                    (fun () -> databricks { key_vault_key_version Guid.Empty } |> ignore)
                ...
                Expect.throws
                    (fun () -> databricks { encrypt_with_key_vault "test" "test" } |> ignore)
