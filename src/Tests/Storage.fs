...
let tests =
    testList "Storage Tests" [
            ...
            test "Metadata is added correctly to multiple queues" {
                let resources: StorageQueue list =
                    let account = storageAccount {
                        ...
                        add_queues [ storageQueue { name "queue3" }; storageQueue { name "queue4" } ] [
                            "environment", "test"
                            "project", "barnyard"
                        ]
                    }
        ...
        test "Creates Role Assignment correctly" {
            ...
            let roleAssignment =
                builder.BuildResources Location.NorthEurope |> List.last :?> Farmer.Arm.RoleAssignment.RoleAssignment
