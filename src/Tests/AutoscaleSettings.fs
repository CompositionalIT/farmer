...
let tests =
    testList "Autoscale Settings" [
        test "Basic Autoscale Settings ARM Resource (no builder)" {
            ...
            Expect.equal (profile.SelectToken "capacity.default" |> string) "1" "Incorrect profile[0].capacity.default"

            Expect.equal (profile.SelectToken "capacity.maximum" |> string) "10" "Incorrect profile[0].capacity.maximum"

            Expect.equal (profile.SelectToken "capacity.minimum" |> string) "1" "Incorrect profile[0].capacity.minimum"
