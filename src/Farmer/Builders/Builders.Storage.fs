...
type StorageAccount =
    ...
    static member getConnectionString(storageAccountName: StorageAccountName, ?group) =
        ...
        StorageAccount
            .getConnectionString(resourceId)
...
type StorageAccountBuilder() =
    ...
    member _.AddQueue(state: StorageAccountConfig, name: string) =
        { state with Queues = state.Queues.Add(StorageResourceName.Create(name).OkValue) }
    ...
    member this.AddQueues(state: StorageAccountConfig, names) =
        (state, names)
        ||> Seq.fold (fun state name -> this.AddQueue(state, name))
    ...
    member _.AddTable(state: StorageAccountConfig, name: string) =
        { state with Tables = state.Tables.Add(StorageResourceName.Create(name).OkValue) }
    ...
    member this.AddTables(state: StorageAccountConfig, names) =
        (state, names)
        ||> Seq.fold (fun state name -> this.AddTable(state, name))
    ...
    member _.UseHns(state: StorageAccountConfig, value) =
        { state with EnableDataLake = Some value }
    ...
    static member private GrantAccess(state: StorageAccountConfig, assignment) =
        { state with RoleAssignments = state.RoleAssignments.Add assignment }
    ...
    member _.SetNetworkAcls(state: StorageAccountConfig, networkAcls) =
        { state with NetworkAcls = Some networkAcls }
    ...
    member _.RestrictToSubnet(state: StorageAccountConfig, vnet: string, subnet: string) =
        ...
        | Some existingAcl ->
            { state with
                NetworkAcls =
                    { existingAcl with VirtualNetworkRules = allowVnet :: existingAcl.VirtualNetworkRules }
                    |> Some }
    ...
    member _.RestrictToPrefix(state: StorageAccountConfig, cidr: string) =
        ...
        | Some existingAcl ->
            { state with
                NetworkAcls =
                    { existingAcl with IpRules = allowIp :: existingAcl.IpRules }
                    |> Some }
