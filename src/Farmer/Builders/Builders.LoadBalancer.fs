...
type BackendAddressPoolBuilder () =
    ...
    member _.IpAddresses(state:BackendAddressPoolConfig, backendAddresses:string list) =
        { state with LoadBalancerBackendAddresses = state.LoadBalancerBackendAddresses @ (backendAddresses |> List.map System.Net.IPAddress.Parse)  }
    member _.IpAddresses(state:BackendAddressPoolConfig, backendAddresses:System.Net.IPAddress list) =
        { state with LoadBalancerBackendAddresses = state.LoadBalancerBackendAddresses @ (backendAddresses)  }
...
type ProbeBuilder () =
    ...
    member _.RequestPath(state:ProbeConfig, requestPath:string) = { state with RequestPath = Some requestPath }
    /// Sets the interval in seconds between probes.
    [<CustomOperation "interval">]
    member _.Interval(state:ProbeConfig, interval:int) = { state with IntervalInSeconds = interval }
    member _.Interval(state:ProbeConfig, interval:TimeSpan) = { state with IntervalInSeconds = int interval.TotalSeconds }
    /// Sets the number of probes to consider this backend a failure and remove from the pool.
    [<CustomOperation "number_of_probes">]
    member _.NumberOfProbes(state:ProbeConfig, numberOfProbes) = { state with NumberOfProbes = numberOfProbes }
...
type LoadBalancingRuleBuilder () =
    ...
    member _.FrontendIpConfig(state:LoadBalancingRuleConfig, frontendIpConfig:string) = { state with FrontendIpConfiguration = ResourceName frontendIpConfig }
    member _.FrontendIpConfig(state:LoadBalancingRuleConfig, frontendIpConfig:FrontendIpConfig) = { state with FrontendIpConfiguration = frontendIpConfig.Name }
    /// Sets the name of the load balancing rule.
    [<CustomOperation "backend_address_pool">]
    member _.BackendAddressPool(state:LoadBalancingRuleConfig, backendAddressPool:string) = { state with BackendAddressPool = ResourceName backendAddressPool }
    member _.BackendAddressPool(state:LoadBalancingRuleConfig, backendAddressPool:BackendAddressPoolConfig) = { state with BackendAddressPool = backendAddressPool.Name }
    /// Sets the probe to use for this load balancing rule.
    [<CustomOperation "probe">]
    member _.Probe(state:LoadBalancingRuleConfig, probe:string) = { state with Probe = Some (ResourceName probe) }
    ...
    member _.FrontendPort(state:LoadBalancingRuleConfig, frontendPort:uint16) = { state with FrontendPort = frontendPort }
    member _.FrontendPort(state:LoadBalancingRuleConfig, frontendPort:int) = { state with FrontendPort = uint16 frontendPort }
    ...
    member _.BackendPort(state:LoadBalancingRuleConfig, backendPort:int) = { state with BackendPort = uint16 backendPort }
    /// Sets the load balancing protocol for this rule
    [<CustomOperation "protocol">]
    member _.Protocol(state:LoadBalancingRuleConfig, protocol:TransmissionProtocol) = { state with Protocol = Some protocol }
    ...
    member _.LoadDistributionPolicy(state:LoadBalancingRuleConfig, loadDistributionPolicy:LoadDistributionPolicy) =
        { state with LoadDistribution = loadDistributionPolicy }
    ...
    member _.EnableTcpReset(state:LoadBalancingRuleConfig) =
        { state with EnableTcpReset = Some true }
    ...
    member _.EnableOutboundSnat(state:LoadBalancingRuleConfig) =
        { state with DisableOutboundSnat = Some false }
...
type LoadBalancerBuilder () =
    ...
    [<CustomOperation "sku">]
    member _.Sku(state:LoadBalancerConfig, skuName) = { state with Sku = { state.Sku with Name = skuName } }
    /// Sets the tier of the load balancer (default is 'regional').
    [<CustomOperation "tier">]
    member _.Tier(state:LoadBalancerConfig, skuTier) = { state with Sku = { state.Sku with Tier = skuTier } }
    /// Add one or more frontend IP configs.
    [<CustomOperation "add_frontends">]
    member _.AddFrontends(state:LoadBalancerConfig, frontends) = { state with FrontendIpConfigs = state.FrontendIpConfigs @ frontends }
    /// Add one or more backend pools.
    [<CustomOperation "add_backend_pools">]
    member _.AddBackendPools(state:LoadBalancerConfig, backends) = { state with BackendAddressPools = state.BackendAddressPools @ backends }
    /// Add one or more load balancing rules.
    [<CustomOperation "add_rules">]
    member _.AddLoadBalancingRules(state:LoadBalancerConfig, rules) = { state with LoadBalancingRules = state.LoadBalancingRules @ rules }
