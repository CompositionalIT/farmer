...
type SecurityRuleBuilder() =
    ...
    member _.AddSource(state: SecurityRuleConfig, source) =
        { state with Sources = source :: state.Sources }
    ...
    member _.AddSourceAny(state: SecurityRuleConfig, protocol) =
        { state with Sources = (protocol, AnyEndpoint, AnyPort) :: state.Sources }
    ...
    member _.AddSourceTag(state: SecurityRuleConfig, protocol, tag) =
        { state with Sources = (protocol, Tag tag, AnyPort) :: state.Sources }
    ...
    member _.AddSourceAddress(state: SecurityRuleConfig, protocol, sourceAddress: string) =
        { state with
            Sources =
                (protocol, Host(IPAddress.Parse sourceAddress), AnyPort)
                :: state.Sources }
    ...
    member _.AddSourceNetwork(state: SecurityRuleConfig, protocol, sourceNetwork) =
        { state with
            Sources =
                (protocol, Network(IPAddressCidr.parse sourceNetwork), AnyPort)
                :: state.Sources }
    ...
    member _.AddDestination(state: SecurityRuleConfig, dest) =
        { state with Destinations = dest :: state.Destinations }
    ...
    member _.AddDestinationAny(state: SecurityRuleConfig) =
        { state with Destinations = AnyEndpoint :: state.Destinations }
    ...
    member _.AddDestinationTag(state: SecurityRuleConfig, tag) =
        { state with Destinations = Tag tag :: state.Destinations }
...
let internal buildNsgRule (nsgName: ResourceName) (rule: SecurityRuleConfig) (priority: int) =
    { Name = rule.Name
      ...
      Protocol =
        let protocols =
            rule.Sources
            |> List.map (fun (protocol, _, _) -> protocol)
            |> Set
      ...
      SourcePorts =
        rule.Sources
        |> List.map (fun (_, _, sourcePort) -> sourcePort)
        |> Set
      SourceAddresses =
        rule.Sources
      ...
      DestinationPorts =
        ...
        | services ->
            services
            |> List.map (fun (NetworkService (_, port)) -> port)
            |> Set
...
type NsgBuilder() =
    ...
    member _.AddSecurityRules(state: NsgConfig, rules) =
        { state with SecurityRules = state.SecurityRules @ rules }
