...
type ResourceGroupConfig =
    { TargetResourceGroup: string Option
      ...
      Mode: DeploymentMode
      ArmMapper : (IArmResource -> obj option)
...
type DeploymentBuilder() =
    member _.Yield _ =
        { TargetResourceGroup = None
          ...
          Mode = Incremental
          ArmMapper = fun _ -> None
          Tags = Map.empty }
           
    member this.Run (state:ResourceGroupConfig) =
        { state with
            Resources = [
                for resource in state.Resources do
                    match state.ArmMapper resource with
                    | Some json ->
                        { new IArmResource with
                            member _.ResourceId = resource.ResourceId
                            member _.JsonModel = json }
                    | None ->
                        resource
            ]
        }
        
    [<CustomOperation "rewriter">]
    member _.Rewriter (state:ResourceGroupConfig, rewriter) = { state with ArmMapper = rewriter } 
    member this.Rewriter (state:ResourceGroupConfig, rewriter) =
        this.Rewriter(
            state,
            fun r ->
                let json = JsonSerializer.Serialize r.JsonModel
                rewriter r.ResourceId json |> Option.map (fun (j:string) -> JsonSerializer.Deserialize j)) 
