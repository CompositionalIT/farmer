        ...
        if this.Parameters.IsEmpty
           ...
           && this.Resources.IsEmpty then
            ...
            { ResourceGroupDeployment.TargetResourceGroup =
                ...
                |> Option.defaultValue "farmer-deploy"
                |> ResourceName
...
type DeploymentBuilder() =
    ...
    member _.Output(state, outputName, outputValue) : ResourceGroupConfig =
        { state with Outputs = state.Outputs.Add(outputName, outputValue) }
    ...
    member __.Outputs(state, outputs) : ResourceGroupConfig =
        { state with Outputs = Map.merge outputs state.Outputs }

    member this.Outputs(state: ResourceGroupConfig, outputs) =
        this.Outputs(
            state,
            outputs
            |> List.map (fun (k: string, ResourceName r) -> k, r)
        )
    ...
    member this.Outputs(state: ResourceGroupConfig, outputs) =
        this.Outputs(
            state,
            outputs
            |> List.map (fun (k, a: ArmExpression) -> k, a.Eval())
        )
    ...
    member this.AddResources(state: ResourceGroupConfig, input: IBuilder list) =
        let resources =
            input
            |> List.collect (fun i -> i.BuildResources state.Location)
...
type ResourceGroupBuilder() =
    ...
    member this.AddParameterValues(state: ResourceGroupConfig, parameters: (string * string) list) =
        { state with
            ParameterValues =
                ...
                @ (parameters |> List.map ParameterValue) }
    ...
    member this.AddKeyVaultSecretReferences
        ...
        { state with
            ParameterValues =
                state.ParameterValues
                @ (parameters |> List.map KeyVaultReference) }
