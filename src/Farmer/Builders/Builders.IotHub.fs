...
type IotHubConfig =
    ...
    member this.GetConnectionString policy =
        ...
        let expr =
            $"concat('Endpoint=',{endpoint},';SharedAccessKeyName={policy.ToString().ToLower()};SharedAccessKey=',{this.BuildKey policy})"
...
type IotHubBuilder() =
    ...
    member _.PartitionCount(state: IotHubConfig, partitions) =
        { state with PartitionCount = Some partitions }
    ...
    member _.DeviceProvisioning(state: IotHubConfig) =
        { state with DeviceProvisioning = Enabled }
