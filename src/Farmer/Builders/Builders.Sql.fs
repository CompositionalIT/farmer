    ...
    interface IBuilder with
        ...
        member this.BuildResources location =
            [ let elasticPoolName =
              ...
              if this.Databases
                 |> List.exists (fun db -> db.Sku.IsNone) then
                  { Name = elasticPoolName
                    Server = this.Name
                    Location = location
                    Sku = this.ElasticPoolSettings.Sku
                    MaxSizeBytes =
                      this.ElasticPoolSettings.Capacity
                      |> Option.map Mb.toBytes
                    MinMax =
                      this.ElasticPoolSettings.PerDbLimits
                      |> Option.map (fun l -> l.Min, l.Max) }
              ...
              | Some replica ->
                  if replica.Location.ArmValue = location.ArmValue then
                      ...
                      let replicaServerName =
                          match (this.Name.ResourceName.Value + replica.NameSuffix)
                                |> SqlAccountName.Create
                              with
                          | Ok x -> x
...
type SqlDbBuilder() =
    ...
    member _.DbSku(state: SqlAzureDbConfig, sku: MSeries) =
        { state with Sku = Some(VCore(MemoryIntensive sku, LicenseRequired)) }

    member _.DbSku(state: SqlAzureDbConfig, sku: FSeries) =
        { state with Sku = Some(VCore(CpuIntensive sku, LicenseRequired)) }

    member _.DbSku(state: SqlAzureDbConfig, sku: VCoreSku) =
        { state with Sku = Some(VCore(sku, LicenseRequired)) }
...
type SqlServerBuilder() =
    ...
    member _.AddDatabases(state: SqlAzureConfig, databases) =
        { state with Databases = state.Databases @ databases }
