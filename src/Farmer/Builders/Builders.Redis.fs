...
type RedisBuilder() =
    ...
    member _.Run(state: RedisConfig) =
        { state with
            ...
            ShardCount =
                match state with
                | { Sku = Premium
                    ShardCount = Some shards } when shards > 10 -> Some 10
    ...
    member _.AddSetting(state: RedisConfig, key, value) =
        { state with RedisConfiguration = state.RedisConfiguration.Add(key, value) }
