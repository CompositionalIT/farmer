...
type ServiceBusBuilder() =
    ...
    member _.DependsOn(state:ServiceBusConfig, resourceName:_ ResourceName) = { state with DependsOn = resourceName.Untyped :: state.DependsOn }
    member _.DependsOn(state:ServiceBusConfig, resources:_ ResourceName list) = { state with DependsOn = List.concat [ resources |> List.map(fun r -> r.Untyped); state.DependsOn ] }
