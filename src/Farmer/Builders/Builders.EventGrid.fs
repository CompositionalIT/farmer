...
module SystemEvents =
    ...
    module EventHub =
        let CaptureFileCreated =
            toEvent<EventHubEvent> "Microsoft.EventHub.CaptureFileCreated"
    ...
    module ServiceBus =
        let ActiveMessagesAvailableWithNoListeners =
            toEvent<ServiceBusEvent> "Microsoft.ServiceBus.ActiveMessagesAvailableWithNoListeners"

        let DeadLetterMessagesAvailableWithNoListeners =
            toEvent<ServiceBusEvent> "Microsoft.ServiceBus.DeadLetterMessagesAvailableWithNoListeners"

    module ContainerRegistry =
        let ImagePushed =
            toEvent<ContainerRegistryEvent> "Microsoft.ContainerRegistry.ImagePushed"

        let ImageDeleted =
            toEvent<ContainerRegistryEvent> "Microsoft.ContainerRegistry.ImageDeleted"

        let ChartPushed =
            toEvent<ContainerRegistryEvent> "Microsoft.ContainerRegistry.ChartPushed"

        let ChartDeleted =
            toEvent<ContainerRegistryEvent> "Microsoft.ContainerRegistry.ChartDeleted"
    ...
    module KeyVault =
        let CertificateNewVersionCreated =
            toEvent<KeyVaultEvent> "Microsoft.KeyVault.CertificateNewVersionCreated"

        let CertificateNearExpiry =
            toEvent<KeyVaultEvent> "Microsoft.KeyVault.CertificateNearExpiry"

        let CertificateExpired =
            toEvent<KeyVaultEvent> "Microsoft.KeyVault.CertificateExpired"

        let SecretNewVersionCreated =
            toEvent<KeyVaultEvent> "Microsoft.KeyVault.SecretNewVersionCreated"
        ...
        let KeyNewVersionCreated =
            toEvent<KeyVaultEvent> "Microsoft.KeyVault.KeyNewVersionCreated"
    ...
    module AppService =
        ...
        let BackupOperationStarted =
            toEvent<AppServiceEvent> "Microsoft.Web.BackupOperationStarted"

        let BackupOperationCompleted =
            toEvent<AppServiceEvent> "Microsoft.Web.BackupOperationCompleted"

        let BackupOperationFailed =
            toEvent<AppServiceEvent> "Microsoft.Web.BackupOperationFailed"

        let RestoreOperationStarted =
            toEvent<AppServiceEvent> "Microsoft.Web.RestoreOperationStarted"

        let RestoreOperationCompleted =
            toEvent<AppServiceEvent> "Microsoft.Web.RestoreOperationCompleted"

        let RestoreOperationFailed =
            toEvent<AppServiceEvent> "Microsoft.Web.RestoreOperationFailed"
        ...
        let SlotSwapWithPreviewStarted =
            toEvent<AppServiceEvent> "Microsoft.Web.SlotSwapWithPreviewStarted"

        let SlotSwapWithPreviewCancelled =
            toEvent<AppServiceEvent> "Microsoft.Web.SlotSwapWithPreviewCancelled"

    module SignalR =
        let ClientConnectionConnected =
            toEvent<SignalRServiceEvent> "Microsoft.SignalRService.ClientConnectionConnected"
...
type EventGridBuilder() =
    ...
    member _.Name(state: EventGridConfig<'T>, name) =
        { state with TopicName = ResourceName name }

    [<CustomOperation "source">]
    member _.Source(state: EventGridConfig<_>, source: StorageAccountConfig) =
        EventGridBuilder.ChangeTopic<StorageEvent>(state, source.Name.ResourceName, Topics.StorageAccount)

    member _.Source(state: EventGridConfig<_>, source: WebAppConfig) =
        EventGridBuilder.ChangeTopic<AppServiceEvent>(state, source.Name.ResourceName, Topics.AppService)

    member _.Source(state: EventGridConfig<_>, source: KeyVaultConfig) =
        EventGridBuilder.ChangeTopic<KeyVaultEvent>(state, source.Name, Topics.KeyVault)

    member _.Source(state: EventGridConfig<_>, source: SignalRConfig) =
        EventGridBuilder.ChangeTopic<SignalRServiceEvent>(state, source.Name, Topics.SignalR)

    member _.Source(state: EventGridConfig<_>, source: MapsConfig) =
        EventGridBuilder.ChangeTopic<MapsEvent>(state, source.Name, Topics.MapsAccount)

    member _.Source(state: EventGridConfig<_>, source: ContainerRegistryConfig) =
        EventGridBuilder.ChangeTopic<ContainerRegistryEvent>(state, source.Name, Topics.ContainerRegistry)

    member _.Source(state: EventGridConfig<_>, source: ServiceBusConfig) =
        EventGridBuilder.ChangeTopic<ServiceBusEvent>(state, source.Name, Topics.ServiceBusNamespace)

    member _.Source(state: EventGridConfig<_>, source: IotHubConfig) =
        EventGridBuilder.ChangeTopic<IoTHubEvent>(state, source.Name, Topics.IoTHubAccount)

    member _.Source(state: EventGridConfig<_>, source: EventHubConfig) =
        EventGridBuilder.ChangeTopic<EventHubEvent>(state, source.EventHubNamespaceName, Topics.EventHubsNamespace)
    ...
    member _.AddWebSubscription(state: EventGridConfig<'T>, webAppName: ResourceName, webHookEndpoint: Uri, events) =
        EventGridBuilder.AddSub(
            state,
            webHookEndpoint.LocalPath + "-webhook",
            webAppName,
    ...
    member this.AddWebSubscription(state: EventGridConfig<_>, webApp: WebAppConfig, route, events) =
        this.AddWebSubscription(state, webApp.Name.ResourceName, Uri $"https://{webApp.Endpoint}/%s{route}", events)
    ...
    member _.AddEventHubSubscription
        ...
        EventGridBuilder.AddSub(
            ...
            eventHub.EventHubNamespaceName,
    ...
    member _.Tags(state: EventGridConfig<'T>, pairs) =
        { state with
            Tags =
                pairs
                |> List.fold (fun map (key, value) -> Map.add key value map) state.Tags }
