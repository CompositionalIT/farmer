...
type DeploymentScriptBuilder() =
    ...
    member _.CleanupPreference(state: DeploymentScriptConfig) = {
        state with
            CleanupPreference = Cleanup.OnSuccess
    }
    ...
    member _.Content(state: DeploymentScriptConfig, content) = {
        state with
    ...
    member _.PrimaryScriptUri(state: DeploymentScriptConfig, primaryScriptUri) = {
        state with
            ScriptSource = Remote primaryScriptUri
    }
    ...
    member _.EnvironmentVariables(state: DeploymentScriptConfig, envVars) = {
        state with
            EnvironmentVariables = Map envVars
    }
    ...
    member _.ForceUpdate(state: DeploymentScriptConfig) = { state with ForceUpdate = true }

    member _.ForceUpdate(state: DeploymentScriptConfig, value) = { state with ForceUpdate = value }
    ...
    member _.Identity(state: DeploymentScriptConfig, identity) = {
        state with
            CustomIdentity = Some identity
    }

    member this.Identity(state, identity: UserAssignedIdentityConfig) =
        this.Identity(state, identity.UserAssignedIdentity)
    ...
    member _.RetentionInterval(state: DeploymentScriptConfig, retentionInterval) =
        if retentionInterval > 26<Hours> then
            raiseFarmer $"Max retention interval is 26 hours, but was set as {retentionInterval}"

        {
            state with
                CleanupPreference = Cleanup.OnExpiration(TimeSpan.FromHours(float retentionInterval))
        }
    ...
    member _.Timeout(state: DeploymentScriptConfig, timeout) = { state with Timeout = Some timeout }

    /// Timeout for script execution in ISO 8601 format, e.g. PT30M.
    member _.Timeout(state: DeploymentScriptConfig, timeout) = {
        state with
            Timeout = Some(Xml.XmlConvert.ToTimeSpan timeout)
    }

    interface ITaggable<DeploymentScriptConfig> with
        member _.Add state tags = {
            state with
                Tags = state.Tags |> Map.merge tags
        }

    interface IDependable<DeploymentScriptConfig> with
