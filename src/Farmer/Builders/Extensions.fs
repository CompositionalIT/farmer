...
module Extensions =
    module Map =
        let merge newValues map =
            (map, newValues) ||> List.fold (fun map (key, value) -> Map.add key value map)

    type ITaggable<'T> with

        /// Adds the provided set of tags to the builder.
        [<CustomOperation "add_tags">]
        member this.Tags(state: 'T, pairs) = this.Add state pairs
    ...
    type IDependable<'TConfig> with
        ...
        member this.DependsOn(state: 'TConfig, builder: IBuilder) =
        ...
        member this.DependsOn(state: 'TConfig, resources: LinkedResource list) =
            this.Add
                state
                (Set(
                    resources
    ...
    type IPrivateEndpoints<'TConfig> with
        ...
        member this.AddPrivateEndpoint(state, (subnet, name)) =
            this.Add state (Set.singleton (subnet, Some name))

        member this.AddPrivateEndpoint(state, subnet) =
            this.Add state (Set.singleton (subnet, None))

        [<CustomOperation "add_private_endpoints">]
        member this.AddPrivateEndpoints(state, subnets) =
            this.Add state (subnets |> Set.map (fun (sn, ep) -> (sn, Some ep)))

        member this.AddPrivateEndpoints(state, subnets) =
            this.Add state (subnets |> Set.map (fun sn -> (sn, None)))
