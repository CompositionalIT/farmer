...
type FrontendIpBuilder() =
    ...
    member _.LinkToPublicIp(state: FrontendIpConfig, publicIp: string) =
        { state with PublicIp = Some(Unmanaged(virtualNetworks.resourceId (ResourceName publicIp))) }
    ...
    member _.AddAuthCerts(state: BackendHttpSettingsConfig, authCerts) =
        { state with
            AuthenticationCertificates =
                ...
                @ (authCerts
                   |> List.map (fun authCert -> ResourceName authCert)) }
    ...
    member _.TrustedRootCertificates(state: BackendHttpSettingsConfig, trustedRootCerts) =
        { state with
            TrustedRootCertificates =
                ...
                @ (trustedRootCerts
                   |> List.map (fun rootCert -> ResourceName rootCert)) }
        ...
        member this.BuildResources location =
            ...
            { Name = this.Name
              ...
              GatewayIPConfigurations =
                this.GatewayIpConfigs
                |> List.map GatewayIpConfig.BuildResource
              FrontendIpConfigs =
                this.FrontendIpConfigs
                |> List.map FrontendIpConfig.BuildResource
              FrontendPorts =
                this.FrontendPorts
                |> List.map FrontendPortConfig.BuildResource
              BackendAddressPools =
              ...
              HttpListeners =
                this.HttpListeners
                |> List.map HttpListenerConfig.BuildResource
              Probes =
                this.Probes
                |> List.map AppGatewayProbeConfig.BuildResource
              RequestRoutingRules =
                this.RequestRoutingRules
                |> List.map RequestRoutingRuleConfig.BuildResource
...
type AppGatewayBuilder() =
    ...
    member _.AddBackendHttpSettingsCollection
        (
            state: AppGatewayConfig,
            backendHttpSettings: BackendHttpSettingsConfig list
        ) =
        { state with
            BackendHttpSettingsCollection =
                state.BackendHttpSettingsCollection
                @ backendHttpSettings }
