...
type TrafficManagerBuilder() =
    ...
    member _.AddEndpoints(state: TrafficManagerConfig, endpoints: EndpointConfig list) = {
        state with
            EndpointConfigs = state.EndpointConfigs @ endpoints
    }
    ...
    member this.DnsTtl(state: TrafficManagerConfig, ttl: TimeSpan) = {
        state with
            DnsTtl = (int ttl.TotalSeconds) * 1<Seconds>
    }
    ...
    member _.RoutingMethod(state: TrafficManagerConfig, routingMethod) = {
        state with
            RoutingMethod = routingMethod
    }
    ...
    member _.MonitorPort(state: TrafficManagerConfig, port) = {
        state with
            MonitorConfig = { state.MonitorConfig with Port = port }
    }
    ...
    member _.MonitorPath(state: TrafficManagerConfig, path) = {
        state with
            MonitorConfig = { state.MonitorConfig with Path = path }
    }
    ...
    member _.MonitorToleratedFailures(state: TrafficManagerConfig, failures) = {
        state with
            MonitorConfig = {
                state.MonitorConfig with
                    ToleratedNumberOfFailures = failures
            }
    }
