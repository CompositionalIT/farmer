...
type SlotConfig =
    ...
    member this.ToSite(owner: Arm.Web.Site) =
        ...
        let errorMsg =
            $"App name '{owner.Name.Value}' has length={owner.Name.Value.Length}, which exceeds max length ({maxAppNameLengthWhenUsingSlots}) so cannot be used with slots"

        if (owner.Name.Value.Length > maxAppNameLengthWhenUsingSlots) then
            Exceptions.raiseFarmer errorMsg
                    ...
                    fun rg ->
                        ...
                        | Some target ->
                            ...
                            | Some path ->
                                ...
                                let healthCheckDomain =
                                    $"https://%s{owner.Name.Value}-%s{this.Name}.azurewebsites.net/{path}"

                                Console.WriteLine
                                    $"Waiting for %s{owner.Name.Value}/%s{this.Name} to become healthy (GET %s{healthCheckDomain}):"
                                ...
                                | HttpStatusCode.OK -> Some(Ok statusCode)
                                | _ ->
                                    Some(
                                        Error
                                            $"Slot '%s{this.Name}' health check path failed to return OK within %f{maxWaitForHealthy.TotalMinutes} minutes."
                                    )
                            |> function
                                | None
                                | Some (Ok _) ->
                                    Console.Write
                                        $"Swapping slots %s{owner.Name.Value}/%s{this.Name} <-> %s{target} ... "
...
type SlotBuilder() =
    ...
    member this.ManualSwap(state, ?targetSlotName) : SlotConfig =
        { state with
            PostDeploySwapTarget = Some(targetSlotName |> Option.defaultValue "Production")
        }
            ...
            [
                ...
                yield!
                    (PrivateEndpoint.create
                        ...
                        [ "sites" ]
                        Set.empty
                        this.CommonWebConfig.PrivateEndpoints)
            ]
    ...
    member _.Run(state: WebAppConfig) =
        ...
        { state with
            SiteExtensions =
                ...
                | { AutomaticLoggingExtension = false } -> state.SiteExtensions.Remove WebApp.Extensions.Logging
        ...
        member this.AddSlotSettings(state: 'T, settings: (string * string) list) =
            ...
            |> List.fold
                (fun (state: CommonWebConfig) (key, value: string) ->
                    { state with
                        Settings = state.Settings.Add(key, LiteralSetting value)
                        SlotSettingNames = state.SlotSettingNames.Add(key)
                    })
                current
            |> this.Wrap state
