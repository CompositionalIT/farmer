...
type WebAppBuilder() =
    member __.Yield _ =
        { Name = WebApp.tryCreateResourceName "default" |> Result.get
          ServicePlan = derived (fun t -> t.Name.Map(sprintf "%s-farm").TryConvert ResourceName.unsafeWrap |> Result.get )
          AppInsights = Some (derived (fun t -> t.Name.Map(sprintf "%s-ai").TryConvert ResourceName.unsafeWrap |> Result.get))
          Sku = Sku.F1
          WorkerSize = Small
          WorkerCount = 1
