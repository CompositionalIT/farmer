...
type SlotConfig =
    ...
    member this.ToSite(owner: Arm.Web.Site) =
        { owner with
            ...
            Dependencies =
                owner.Dependencies
                |> Set.add (owner.ResourceType.resourceId owner.Name)
            AutoSwapSlotName = this.AutoSwapSlotName
            AppSettings =
                owner.AppSettings
                |> Option.map (Map.merge (this.AppSettings |> Map.toList))
            ConnectionStrings =
            ...
            KeyVaultReferenceIdentity =
                ...
                |> Option.orElse owner.KeyVaultReferenceIdentity
...
type SlotBuilder() =
    ...
    member this.AutoSlotSwapName(state, autoSlotSwapName) : SlotConfig =
        { state with AutoSwapSlotName = Some autoSlotSwapName }
    ...
    member _.AllowIp(state, name, cidr: IPAddressCidr) : SlotConfig =
        { state with
            IpSecurityRestrictions =
                state.IpSecurityRestrictions
                @ [ IpSecurityRestriction.Create name cidr Allow ] }
    ...
    member _.DenyIp(state, name, cidr: IPAddressCidr) : SlotConfig =
        { state with
            IpSecurityRestrictions =
                state.IpSecurityRestrictions
                @ [ IpSecurityRestriction.Create name cidr Deny ] }
...
type VirtualApplicationBuilder() =
    ...
    member _.VirtualPath(state, virtualPath) : VirtualApplicationConfig =
        { state with VirtualPath = virtualPath }

    [<CustomOperation "physical_path">]
    member _.PhysicalPath(state, physicalPath) : VirtualApplicationConfig =
        { state with PhysicalPath = physicalPath }

    [<CustomOperation "preloaded">]
    member _.Preloaded state : VirtualApplicationConfig =
        { state with PreloadEnabled = Some true }
        ...
        member this.BuildResources location =
            [ let keyVault, secrets =
              ...
              let siteSettings =
                  let literalSettings =
                      [ if this.RunFromPackage then
                        ...
                        yield!
                            this.DockerPort
                            |> Option.mapList AppSettings.WebsitesPort
              ...
              let site =
                  { SiteType = Site this.Name
                    ...
                    Metadata =
                      ...
                      | AspNet _, _
                      | DotNet "5.0", Windows -> Some "dotnet"
                      | _ -> None
              ...
              if Map.isEmpty this.CommonWebConfig.Slots then
                  ...
                  { site with
                      AppSettings = None
                      ConnectionStrings = None } // Don't deploy production slot settings as they could cause an app restart
              ...
              for customDomain in this.CustomDomains |> Map.toSeq |> Seq.map snd do
                  ...
                  yield!
                      ((hostNameBindingDeployment :> IBuilder)
                          .BuildResources location)
                  ...
                  | SecureDomain (customDomain, certOptions) ->
                      ...
                      let certificateDeployment =
                          resourceGroup {
                              name (
                                  aspRgName
                                  |> Option.defaultValue "[resourceGroup().name]"
                              )
                      ...
                      yield!
                          ((certificateDeployment :> IBuilder)
                              .BuildResources location)
                      ...
                      yield!
                          ((hostNameCertificateLinkingDeployment :> IBuilder)
                              .BuildResources location)
...
type WebAppBuilder() =
        ...
        { state with
            SiteExtensions =
                ...
                | { Runtime = DotNetCore _
                    AutomaticLoggingExtension = true
                    DockerImage = None
                    CommonWebConfig = { OperatingSystem = Windows } } -> state.SiteExtensions.Add Extensions.Logging
    ...
    member _.Sku(state: WebAppConfig, sku) =
        { state with CommonWebConfig = { state.CommonWebConfig with Sku = sku } }
    ...
    member _.NumberOfWorkers(state: WebAppConfig, workerCount) =
    ...
    member _.NodeVersion(state: WebAppConfig, version) =
        { state with WebsiteNodeDefaultVersion = Some version }

    /// Enables HTTP 2.0 for this webapp.
    [<CustomOperation "enable_http2">]
    member _.Http20Enabled(state: WebAppConfig) =
        { state with HTTP20Enabled = Some true }
    ...
    member _.AddCustomDomain(state: WebAppConfig, domainConfig: DomainConfig) =
        { state with
            CustomDomains =
                state.CustomDomains
                |> Map.add domainConfig.DomainName domainConfig }
    ...
    member _.DockerPort(state: WebAppConfig, dockerPort: int) =
        { state with DockerPort = Some dockerPort }

    /// Enables the zone redundancy in service plan
    [<CustomOperation "zone_redundant">]
    member this.ZoneRedundant(state: WebAppConfig, flag: FeatureFlag) =
            ...
            { state with
                CommonWebConfig =
                    { state.CommonWebConfig with
                        PrivateEndpoints =
                            state.CommonWebConfig.PrivateEndpoints
                            |> Set.union endpoints } }
...
module Extensions =
    type IServicePlanApp<'T> with
        ...
        member this.SetServicePlanName(state: 'T, name) =
            { this.Get state with ServicePlan = named serverFarms name }
            |> this.Wrap state
        ...
        member this.LinkToServicePlan(state: 'T, name) =
            { this.Get state with ServicePlan = managed serverFarms name }
            |> this.Wrap state
        ...
        member this.LinkToUnmanagedServicePlan(state: 'T, resourceId) =
            { this.Get state with ServicePlan = unmanaged resourceId }
            |> this.Wrap state
        ...
        member this.UseAppInsights(state: 'T, name) =
            { this.Get state with AppInsights = Some(named components name) }
            |> this.Wrap state
        ...
        member this.DeactivateAppInsights(state: 'T) =
            { this.Get state with AppInsights = None }
            |> this.Wrap state
        ...
        member this.LinkToAi(state: 'T, name) =
            { this.Get state with AppInsights = Some(managed components name) }
            |> this.Wrap state
        ...
        member this.LinkUnmanagedAppInsights(state: 'T, resourceId) =
            { this.Get state with AppInsights = Some(unmanaged resourceId) }
            |> this.Wrap state
        ...
        member this.AddConnectionString(state: 'T, key) =
            ...
            { current with
                ConnectionStrings = current.ConnectionStrings.Add(key, (ParameterSetting(SecureParameter key), Custom)) }
            |> this.Wrap state
        ...
        member this.EnableCors(state: 'T, origins) =
            { this.Get state with Cors = Some origins }
            |> this.Wrap state
        ...
        member this.OperatingSystem(state: 'T, os) =
            { this.Get state with OperatingSystem = os }
            |> this.Wrap state
        ...
        member this.ZipDeploy(state: 'T, path) =
            { this.Get state with ZipDeployPath = Some(path, ZipDeploy.ProductionSlot) }
            |> this.Wrap state

        /// Specifies a folder path or a zip file containing the web application to install as a post-deployment task.
        [<CustomOperation "zip_deploy_slot">]
        member this.ZipDeploySlot(state: 'T, slotName, path) =
            { this.Get state with ZipDeployPath = Some(path, ZipDeploy.NamedSlot slotName) }
            |> this.Wrap state
        ...
        member this.AlwaysOn(state: 'T) =
            { this.Get state with AlwaysOn = true }
            |> this.Wrap state
        ...
        member this.WorkerProcess(state: 'T, bitness) =
            { this.Get state with WorkerProcess = Some bitness }
            |> this.Wrap state
        ...
        member this.UseKeyVault(state: 'T) =
            ...
            { current with
                ...
                SecretStore =
                    KeyVault(derived (fun c -> vaults.resourceId (ResourceName(c.Name.ResourceName.Value + "vault")))) }
            |> this.Wrap state
        ...
        member this.AddSlots(state: 'T, slots: SlotConfig list) =
            ...
            { current with
                Slots =
                    slots
                    |> List.fold (fun m s -> Map.add s.Name s m) current.Slots }
            |> this.Wrap state
        ...
        member this.HealthCheckPath(state: 'T, healthCheckPath: string) =
            this.Map state (fun x -> { x with HealthCheckPath = Some(healthCheckPath) })
        ...
        member this.AllowIp(state: 'T, name, ip: IPAddressCidr) =
            this.Map state (fun x ->
                { x with
                    IpSecurityRestrictions =
                        IpSecurityRestriction.Create name ip Allow
                        :: x.IpSecurityRestrictions })
        ...
        member this.DenyIp(state: 'T, name, ip) =
            this.Map state (fun x ->
                { x with
                    IpSecurityRestrictions =
                        IpSecurityRestriction.Create name ip Deny
                        :: x.IpSecurityRestrictions })
        ...
        member this.LinkToVNet(state: 'T, subnet: SubnetReference option) =
            match subnet with
            | Some subnetId ->
                if subnetId.ResourceId.Type.Type
                   <> Arm.Network.subnets.Type then
