    ...
    interface IBuilder with
        ...
        member this.BuildResources location =
            [ { Location = location
                Name = this.Name
                AvailabilityZone =
                  if this.AvailabilityZone.IsSome
                     && this.NetworkProfile.IsSome then
                ...
                DnsConfig =
                  if this.DnsConfig.IsSome
                     && this.NetworkProfile.IsNone then
                ...
                NetworkProfile =
                  if this.NetworkProfile.IsSome
                     && (this.VirtualNetwork.IsSome
                         || this.SubnetName.IsSome) then
                      raiseFarmer
...
type ContainerGroupBuilder() =
    member private _.AddPort(state, portType, port) : ContainerGroupConfig =
        { state with
            IpAddress =
                ...
                | Some ipAddresses ->
                    { ipAddresses with Ports = ipAddresses.Ports.Add {| Protocol = portType; Port = port |} }
                    |> Some
                | None ->
                    { Type = IpAddressType.PublicAddress
                      Ports =
                        [ {| Protocol = portType; Port = port |} ]
                        |> Set.ofList }
                    |> Some }
    ...
    member _.NetworkProfile(state: ContainerGroupConfig, networkProfileName: string) =
        { state with NetworkProfile = Some(ResourceName networkProfileName) }

    member _.NetworkProfile(state: ContainerGroupConfig, networkProfile: NetworkProfileConfig) =
        { state with NetworkProfile = Some networkProfile.Name }
    ...
    member _.VNetId(state: ContainerGroupConfig, vnetId: ResourceId) =
        { state with VirtualNetwork = Some(Managed vnetId) }

    member _.VNetId(state: ContainerGroupConfig, vnetName: string) =
        { state with VirtualNetwork = Some(Managed(virtualNetworks.resourceId (ResourceName vnetName))) }

    member _.VNetId(state: ContainerGroupConfig, vnetName: ResourceName) =
        { state with VirtualNetwork = Some(Managed(virtualNetworks.resourceId vnetName)) }

    /// Sets the name of a virtual network where this container group should be attached.
    [<CustomOperation "link_to_vnet">]
    member _.LinkToVNetId(state: ContainerGroupConfig, vnetId: ResourceId) =
        { state with VirtualNetwork = Some(Unmanaged vnetId) }

    member _.LinkToVNetId(state: ContainerGroupConfig, vnetName: string) =
        { state with VirtualNetwork = Some(Unmanaged(virtualNetworks.resourceId (ResourceName vnetName))) }

    member _.LinkToVNetId(state: ContainerGroupConfig, vnetName: ResourceName) =
        { state with VirtualNetwork = Some(Unmanaged(virtualNetworks.resourceId vnetName)) }

    [<CustomOperation "subnet">]
    member _.Subnet(state: ContainerGroupConfig, subnetName: string) =
        { state with SubnetName = Some(ResourceName subnetName) }

    member _.Subnet(state: ContainerGroupConfig, subnetName: ResourceName) =
        { state with SubnetName = Some subnetName }
    ...
    member _.AddInitContainers(state: ContainerGroupConfig, initContainers) =
        { state with InitContainers = state.InitContainers @ (Seq.toList initContainers) }
    ...
    member _.AddInstances(state: ContainerGroupConfig, instances) =
        { state with Instances = state.Instances @ (Seq.toList instances) }
        ...
        let dns =
            ...
            | None ->
                { NameServers = nameServers
                  Options = []
                  SearchDomains = [] }
            | Some dnsConfig -> { dnsConfig with NameServers = nameServers }
    ...
    member _.DnsSearchDomains(state: ContainerGroupConfig, searchDomains: string list) =
        let dns =
            ...
            | Some dnsConfig -> { dnsConfig with SearchDomains = searchDomains }

        { state with DnsConfig = Some dns }

    interface IIdentity<ContainerGroupConfig> with
        member _.Add state updater =
            { state with Identity = updater state.Identity }
...
type ContainerInstanceBuilder() =
    ...
    member _.PublicDockerImage(state: ContainerInstanceConfig, containerName: string, version: string) =
        { state with
            Image =
                Containers.DockerImage.PublicImage(containerName, Some version)
                |> Some }
    ...
    member _.Gpu(state: ContainerInstanceConfig, (gpu: ContainerGpuConfig)) =
        { state with Gpu = Some { Count = gpu.Count; Sku = gpu.Sku } }
