...
type VirtualHubBuilder() =
    ...
    member _.AddressPrefix(state: VirtualHubConfig, addressPrefix) =
        { state with
            AddressPrefix = Some addressPrefix
        }
    ...
    member _.LinkToVwan(state: VirtualHubConfig, vwan: VirtualWan) =
        { state with
            Vwan = Some(LinkedResource.Managed (vwan :> IArmResource).ResourceId)
        }

    member _.LinkToVwan(state: VirtualHubConfig, vwan: VirtualWanConfig) =
        { state with
            Vwan = Some(LinkedResource.Managed (vwan :> IBuilder).ResourceId)
        }
    ...
    member _.LinkToExternalVwan(state: VirtualHubConfig, vwanResourceId) =
        { state with
            Vwan = Some(LinkedResource.Unmanaged vwanResourceId)
        }
...
type HubRouteTableBuilder() =
    ...
    member _.Name(state: HubRouteTableConfig, name) =
        { state with
    ...
    member _.LinkToVhub(state: HubRouteTableConfig, vhub: VirtualHub) =
        { state with
            Vhub = LinkedResource.Managed (vhub :> IArmResource).ResourceId
        }

    member _.LinkToVhub(state: HubRouteTableConfig, vhub: VirtualHubConfig) =
        { state with
            Vhub = LinkedResource.Managed (vhub :> IBuilder).ResourceId
        }
    ...
    member _.LinkToExternalVhub(state: HubRouteTableConfig, resourceId) =
        { state with
            Vhub = LinkedResource.Unmanaged resourceId
        }
    ...
    member _.Run(state: HubRouteTableConfig) =
        ...
        | Managed resourceId
        | Unmanaged resourceId when resourceId.Name <> ResourceName.Empty -> state
