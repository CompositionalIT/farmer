...
type VmConfig =
    ...
    member this.IpName = match this.AllocatePublicIp with | Disabled -> None | _ -> Some (this.DeriveResourceName publicIPAddresses "ip")
    member this.Hostname = this.IpName |> Option.map (fun ipName -> ArmExpression.reference(publicIPAddresses, ipName).Map(sprintf "%s.dnsSettings.fqdn"))
            ...
            { Name = this.NicName.Name
              ...
              IpConfigs = [
                {| SubnetName = subnetName.Name
                   PublicIpName = this.IpName |> Option.map (fun x -> x.Name) |} ]
...
type VirtualMachineBuilder() =
    ...
    [<CustomOperation "allocate_public_ip">]
    member _.AllocatePublicIp(state: VmConfig, flag:FeatureFlag) = { state with AllocatePublicIp = flag }
