...
type VmConfig = {
    ...
    PublicIpSku: PublicIpAddress.Sku option
            ...
            [
                ...
                | Some ref -> {
                    ...
                    AllocationMethod =
                        ...
                        | Some PublicIpAddress.Standard -> PublicIpAddress.AllocationMethod.Static
                        | _ ->
...
type VirtualMachineBuilder() =
    ...
    member _.Encryption(state: VmConfig, enabled) = {
        state with
            SecurityProfile =
                ...
                |> (fun x -> {
                    x with
                        EncryptionAtHost = Some enabled
                })
                |> Some
    }

    [<CustomOperation "encryption_identity">]
    member _.Encryption(state: VmConfig, identity) = {
        state with
            SecurityProfile =
                state.SecurityProfile
                |> Option.defaultValue VmSecurityProfile.Default
                |> (fun x -> {
                    x with
                        EncryptionIdentity = Some identity
                })
                |> Some
    }

    [<CustomOperation "proxy_agent">]
    member _.ProxyAgent(state: VmConfig, (keyIncarnationId, mode)) = {
        state with
            SecurityProfile =
                state.SecurityProfile
                |> Option.defaultValue VmSecurityProfile.Default
    ...
    member _.SecurityType(state: VmConfig, securityType) = {
        state with
            SecurityProfile =
                state.SecurityProfile
                |> Option.defaultValue VmSecurityProfile.Default
                |> (fun x -> {
                    x with
                        SecurityType = Some securityType
                })
                |> Some
    }
    ...
    /// Enables setting the SKU for the generated public IP address. As of March 31, 2025, this
    /// will always use "Standard" since Azure will no longer allow new Basic Public IP Addresses.
    [<CustomOperation "public_ip_sku">]
    member _.PublicIpSku(state: VmConfig, sku: PublicIpAddress.Sku) = { state with PublicIpSku = Some sku }
