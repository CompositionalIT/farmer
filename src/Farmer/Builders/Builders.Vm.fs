...
type VmConfig =
    ...
    member this.PublicIpId =
        this.PublicIp
        |> Option.map (fun ref -> ref.resourceId this) //(this.DeriveResourceName publicIPAddresses "ip")
        ...
        member this.BuildResources location =
            [
              ...
              { Name = this.Name
                ...
                PublicKeys =
                  if this.DisablePasswordAuthentication.IsSome
                     ...
                     && this.SshPathAndPublicKeys.IsNone then
              ...
              let nsgId =
                  ...
                  |> Option.map (fun nsg -> nsg.ResourceId)
              ...
              | FeatureFlag.Enabled when
                  this.Image.OS = Linux
                  && this.Identity.SystemAssigned = Disabled
                  ->
...
type VirtualMachineBuilder() =
    ...
    member _.StorageAccountNameExternal(state: VmConfig, name) =
        { state with DiagnosticsStorageAccount = Some(LinkedResource name) }
    ...
    member _.PasswordParameter(state: VmConfig, parameterName) =
        { state with PasswordParameter = Some parameterName }
    ...
    member _.SubnetName(state: VmConfig, name: ResourceName) =
        { state with Subnet = Named(subnets.resourceId name) }
    ...
    member _.LinkToVNet(state: VmConfig, name: ResourceName) =
        { state with VNet = LinkedResource(Managed(virtualNetworks.resourceId name)) }
    ...
    member _.LinkToUnmanagedVNet(state: VmConfig, name: ResourceName) =
        { state with VNet = LinkedResource(Unmanaged(virtualNetworks.resourceId name)) }
    ...
    member _.CustomScriptFiles(state: VmConfig, uris: string list) =
        { state with CustomScriptFiles = uris |> List.map Uri }
    ...
    member _.CustomData(state: VmConfig, customData: string) =
        { state with CustomData = Some customData }
    ...
    member _.DisablePasswordAuthentication(state: VmConfig, disablePasswordAuthentication: bool) =
        { state with DisablePasswordAuthentication = Some disablePasswordAuthentication }

    [<CustomOperation "add_authorized_keys">]
    member _.AddAuthorizedKeys(state: VmConfig, sshObjects: (string * string) list) =
        { state with SshPathAndPublicKeys = Some sshObjects }
    ...
    member _.PublicIp(state: VmConfig, ref: LinkedResource) =
        { state with PublicIp = Some(LinkedResource ref) }

    member _.PublicIp(state: VmConfig, _: Automatic) =
        { state with PublicIp = automaticPublicIp }
    ...
    member _.PrivateIpAllocation(state: VmConfig, ref: PrivateIpAddress.AllocationMethod) =
        { state with PrivateIpAllocation = Some ref }
