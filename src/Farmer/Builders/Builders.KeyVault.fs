...
type SecretConfig =
    ...
    static member internal isValid key =
        let charRulesPassed =
            ...
            key
            |> Seq.forall (fun c -> charRules |> Seq.exists (fun r -> r c))
    ...
    member this.ResourceName =
        this.Vault
        |> Option.map (fun x -> x.Name / this.SecretName)
...
type AccessPolicyBuilder() =
    ...
    member _.ApplicationId(state: AccessPolicyConfig, applicationId) =
        { state with ApplicationId = Some applicationId }
...
type AccessPolicy =
    ...
    static member create(principal: PrincipalId, ?permissions) =
        accessPolicy {
            ...
            secret_permissions (
                permissions
                |> Option.defaultValue Secret.ReadSecrets
            )
        }
...
type KeyVaultBuilder() =
    ...
    member _.EnableVmAccess(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with VirtualMachineAccess = Some Enabled } }

    /// Disallows VM access to the vault.
    [<CustomOperation "disable_vm_access">]
    member _.DisableVmAccess(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with VirtualMachineAccess = Some Disabled } }
    ...
    member _.EnableResourceManagerAccess(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with ResourceManagerAccess = Some Enabled } }
    ...
    member _.DisableResourceManagerAccess(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with ResourceManagerAccess = Some Disabled } }
    ...
    member _.EnableDiskEncryptionAccess(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with AzureDiskEncryptionAccess = Some Enabled } }
    ...
    member _.DisableDiskEncryptionAccess(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with AzureDiskEncryptionAccess = Some Disabled } }
    ...
    member _.EnableRbacAuthorization(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with RbacAuthorization = Some Enabled } }
    ...
    member _.DisableRbacAuthorization(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with RbacAuthorization = Some Disabled } }

    /// Enables VM access to the vault.
    [<CustomOperation "enable_soft_delete">]
    member _.EnableSoftDeletion(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with SoftDelete = Some SoftDeletionOnly } }
    ...
    member _.EnableSoftDeletionWithPurgeProtection(state: KeyVaultBuilderState) =
        { state with Access = { state.Access with SoftDelete = Some SoftDeleteWithPurgeProtection } }
    ...
    member _.EnableRecoveryMode(state: KeyVaultBuilderState) =
        { state with CreateMode = Some SimpleCreateMode.Recover }
    ...
    member _.DisableRecoveryMode(state: KeyVaultBuilderState) =
        { state with CreateMode = Some SimpleCreateMode.Default }
    ...
    member _.EnableBypass(state: KeyVaultBuilderState) =
        { state with NetworkAcl = { state.NetworkAcl with Bypass = Some AzureServices } }
    ...
    member _.DisableBypass(state: KeyVaultBuilderState) =
        { state with NetworkAcl = { state.NetworkAcl with Bypass = Some NoTraffic } }
    ...
    member _.AllowDefaultTraffic(state: KeyVaultBuilderState) =
        { state with NetworkAcl = { state.NetworkAcl with DefaultAction = Some Allow } }
    ...
    member _.DenyDefaultTraffic(state: KeyVaultBuilderState) =
        { state with NetworkAcl = { state.NetworkAcl with DefaultAction = Some Deny } }
    ...
    member _.AddIpRule(state: KeyVaultBuilderState, ipRule) =
        { state with NetworkAcl = { state.NetworkAcl with IpRules = ipRule :: state.NetworkAcl.IpRules } }
    ...
    member _.AddVnetRule(state: KeyVaultBuilderState, vnetRule) =
        { state with NetworkAcl = { state.NetworkAcl with VnetRules = vnetRule :: state.NetworkAcl.VnetRules } }
    ...
    member _.AddSecret(state: KeyVaultBuilderState, key: SecretConfig) =
        { state with Secrets = key :: state.Secrets }
    ...
    member this.AddSecrets(state: KeyVaultBuilderState, items) =
        this.AddSecrets(
            state,
            items
            |> Seq.map (fun (key, value) -> SecretConfig.create (key, value))
        )
...
type KeyBuilder() =
    ...
    member _.LinkToKeyVault(state: KeyConfig, keyVault: ResourceId) =
        { state with Vault = Some(Unmanaged keyVault) }

    member _.LinkToKeyVault(state: KeyConfig, keyVault: KeyVaultConfig) =
        { state with Vault = Some(Unmanaged keyVault.ResourceId) }
