...
type EndpointBuilder() =
    ...
    member _.QueryStringCachingBehaviour(state: EndpointConfig, behaviour) =
        { state with QueryStringCachingBehaviour = behaviour }
    ...
    member _.CustomDomain(state: EndpointConfig, hostname) =
        { state with CustomDomain = Some hostname }
...
type CdnRuleBuilder() =
    ...
    member _.WhenRequestProtocol(state: CdnRuleConfig, operator, value) =
        { state with
            Conditions =
                state.Conditions
                @ [ RequestProtocol {| Operator = operator; Value = value |} ] }
    ...
    member _.CacheExpiration(state: CdnRuleConfig, cacheBehaviour) =
        { state with
            Actions =
                state.Actions
                @ [ CacheExpiration {| CacheBehaviour = cacheBehaviour |} ] }
