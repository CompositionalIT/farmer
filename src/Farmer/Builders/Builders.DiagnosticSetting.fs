...
type DiagnosticSettingsBuilder() =
    ...
    member _.Run(state: DiagnosticSettingsConfig) =
        let (|EmptySet|_|) theSet =
            if Set.isEmpty theSet then Some EmptySet else None

        match state with
        | { Metrics = EmptySet; Logs = EmptySet } -> raiseFarmer "You must specify at least one metric or log setting."
        | _ when
            state.Sinks.EventHub = None
            && state.Sinks.StorageAccount = None
            && state.Sinks.LogAnalyticsWorkspace = None
            ->
    ...
    member _.AddDestination(state: DiagnosticSettingsConfig, storageAccount: StorageAccountConfig) =
        DiagnosticSettingsBuilder.AddDestinationPrivate(
            state,
            storageAccounts.resourceId storageAccount.Name.ResourceName
        )
    ...
    member this.Metrics(state, metrics) =
        this.Metrics(state, metrics |> Seq.map MetricSetting.Create)
    ...
    member this.Logs(state, logs: seq<string>) =
        this.Logs(state, logs |> Seq.map LogSetting.Create)

    member this.Logs(state, logs: seq<LogCategory>) =
        this.Logs(state, logs |> Seq.map LogSetting.Create)
