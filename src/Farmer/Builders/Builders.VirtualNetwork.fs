...
type SubnetBuilder() =
    ...
    member _.LinkToRouteTable(state: SubnetConfig, routeTable: ResourceId) =
        { state with
            RouteTable = Some(Managed routeTable)
        }

    [<CustomOperation "link_to_unmanaged_route_table">]
    member _.LinkToUnmanagedRouteTable(state: SubnetConfig, routeTable: ResourceId) =
        { state with
...
type SubnetSpecBuilder() =
    ...
    [<CustomOperation "link_to_route_table">]
    member _.LinkToRouteTable(state: SubnetBuildSpec, routeTableReference) =
        { state with
            RouteTable = routeTableReference
        }

    member _.LinkToRouteTable(state: SubnetBuildSpec, routeTable: ResourceId) =
        { state with
            RouteTable = Some(Managed routeTable)
        }

    [<CustomOperation "link_to_unmanaged_route_table">]
    member _.LinkToUnmanagedRouteTable(state: SubnetBuildSpec, routeTable: ResourceId) =
        { state with
            RouteTable = Some(Unmanaged routeTable)
        }
            ...
            let endpoint =
                {
                    ...
                    Subnet =
                        ...
                        | Some sn -> sn
                        | None -> raiseFarmer "Must have linked subnet"
                    Location = location
...
type PrivateEndpointBuilder() =
    ...
    [<CustomOperation "name">]
    member _.Name(state: PrivateEndpointConfig, name: string) =
        { state with
            Name = (ResourceName name)
        }
    ...
    member _.Subnet(state: PrivateEndpointConfig, subnet: SubnetConfig) =
        { state with
            Subnet = Some(SubnetReference.create (subnet))
        }
        ...
        let outputResource, groupIds, dependencies =
            ...
            | "Microsoft.Web/sites/slots" ->
                ...
                | Some slotName ->
                    ...
                    [ $"sites-%s{slotName.Value}" ],
                    [ resource.ResourceId ]
                | None ->
                    raiseFarmer
                        $"Invalid private endpoint configuration. Slots must have a slot name %s{resource.ResourceId.Type.Type}"
            | _ ->
                raiseFarmer
                    $"Invalid resource type. Cannot link private endpoint to type %s{resource.ResourceId.Type.Type}"
    ...
    [<CustomOperation "link_to_resource">]
    member _.PrivateLinkConnection
        (
            state: PrivateEndpointConfig,
            resource: LinkedResource,
            subresource: string list,
    ...
    member _.LinkToDnsZone(state: PrivateEndpointConfig, zone: DnsZoneConfig) =
        { state with
            PrivateDnsZone = Some(Managed (zone :> IBuilder).ResourceId)
        }

    [<CustomOperation "link_to_unmanaged_private_dns_zone">]
    member _.LinkToDnsZone(state: PrivateEndpointConfig, zone: LinkedResource) =
        { state with
            PrivateDnsZone = Some zone
        }
