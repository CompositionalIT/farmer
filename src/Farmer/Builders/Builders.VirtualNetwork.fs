    ...
    member internal this.AsSubnetResource =
        { Name = this.Name
          ...
          PrivateEndpointNetworkPolicies =
              this.AllowPrivateEndpoints
              |> Option.map FeatureFlag.invert
...
type SubnetBuilder() =
    ...
    member _.Prefix(state: SubnetConfig, prefix) =
        { state with Prefix = IPAddressCidr.parse prefix }
    ...
    member _.PrivateEndpoints(state: SubnetConfig, value: FeatureFlag) =
        { state with AllowPrivateEndpoints = Some value }
...
type AddressSpaceBuilder() =
    ...
    member _.Subnets(state: AddressSpaceSpec, subnets) =
        { state with Subnets = state.Subnets @ subnets }
    ...
    interface IBuilder with
        ...
        member this.BuildResources location =
            [ { Name = this.Name
                ...
                Subnets =
                  this.Subnets
                  |> List.map (fun subnetConfig -> subnetConfig.AsSubnetResource)
                Tags = this.Tags }
              for { RemoteVNet = remote
                    Direction = direction
                    Access = access
                    Transit = transit
                    DependsOn = deps } in this.Peers do
...
type VirtualNetworkBuilder() =
    ...
    member _.AddAddressSpaces(state: VirtualNetworkConfig, prefixes) =
        { state with AddressSpacePrefixes = state.AddressSpacePrefixes @ prefixes }
    ...
    member _.AddPeers(state: VirtualNetworkConfig, peers) =
        { state with Peers = state.Peers @ peers }
    ...
    member this.AddPeers(state: VirtualNetworkConfig, peers: (VirtualNetworkConfig * PeeringMode) list) =
        this.AddPeers(
            state,
            peers
            |> List.map (fun (peer, mode) -> (Managed peer.ResourceId, mode))
        )
    ...
    member _.BuildAddressSpaces(state: VirtualNetworkConfig, addressSpaces: AddressSpaceSpec list) =
        ...
        let newAddressSpaces =
            addressSpaces
            |> List.map (fun addressSpace -> addressSpace.Space)
...
type VNetPeeringSpecBuilder() =
    ...
    member _.VNet(state: VNetPeeringSpec, vnet: VirtualNetworkConfig) =
        { state with RemoteVNet = Managed vnet.ResourceId }
    ...
    interface IDependable<VNetPeeringSpec> with
        member _.Add state resources =
            { state with DependsOn = state.DependsOn |> Set.union resources }
