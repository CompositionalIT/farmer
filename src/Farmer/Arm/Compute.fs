...
let virtualMachines =
    ResourceType("Microsoft.Compute/virtualMachines", "2019-03-01")
...
type VirtualMachine =
    ...
    interface IParameters with
        member this.SecureParameters =
            if this.DisablePasswordAuthentication.IsSome
               && this.DisablePasswordAuthentication.Value then
                []
            else
        ...
        member this.JsonModel =
            let dependsOn =
                [ networkInterfaces.resourceId this.NetworkInterfaceName
                  yield!
                      this.StorageAccount
                      |> Option.mapList storageAccounts.resourceId ]
                ...
                {| priority = this.Priority.ArmValue
                   ...
                   osProfile =
                    {| computerName = this.Name.Value
                       ...
                       adminPassword =
                        if this.DisablePasswordAuthentication.IsSome
                           && this.DisablePasswordAuthentication.Value then //If the disablePasswordAuthentication is set and the value is true then we don't need a password
                            null
                        else
                   ...
                   networkProfile =
                    {| networkInterfaces =
                        [ {| id =
                              networkInterfaces
                                  .resourceId(this.NetworkInterfaceName)
                                  .Eval() |} ] |}
                   diagnosticsProfile =
                    ...
                    | None -> box {| bootDiagnostics = {| enabled = false |} |} |}
