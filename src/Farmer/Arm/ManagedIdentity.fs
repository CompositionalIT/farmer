...
type UserAssignedIdentity =
    ...
    interface IPostDeploy with
        member this.Run rg =
            if this.ActiveDirectoryGroups.IsEmpty then
                None // There are no groups to add this identity to so we can skip the post-deploy action
            else
                result {
                        ...
                        [
                            for group in this.ActiveDirectoryGroups do
                                ...
                                result {
                                    ...
                                    let! inGroup =
                                        az
                                            $"ad group member check -g {group} --member-id {identityId} --query \"value\""
