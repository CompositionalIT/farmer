    ...
    type ZipDeployKind =
        ...
        static member TryParse path =
            if (File.GetAttributes path)
                .HasFlag FileAttributes.Directory then
...
type Site =
    ...
    interface IParameters with
        member this.SecureParameters =
            ...
            @ (optMapToList this.ConnectionStrings
            ...
            let keyvaultId =
                ...
                | None,
                  { SystemAssigned = Disabled
                    UserAssigned = [ x ] } -> x.ResourceId.Eval()
                | _ -> null
                            ...
                            if this.VirtualApplications.IsEmpty then
                                ...
                                |> Seq.map (fun virtualAppKvp ->
                                    {| virtualPath = virtualAppKvp.Key
                                       ...
                                       preloadEnabled =
                                        virtualAppKvp.Value.PreloadEnabled
                                        |> Option.toNullable |})
                                |> box |} |} |}
...
type Certificate =
    ...
    member this.GetThumbprintReference certificateResourceGroup =
        ArmExpression
            .reference({ certificates.resourceId this.ResourceName with ResourceGroup = certificateResourceGroup })
