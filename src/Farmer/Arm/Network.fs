...
type SubnetReference =
    ...
    static member create(vnetRef: LinkedResource, subnetName: ResourceName) =
        if vnetRef.ResourceId.Type.Type
           <> virtualNetworks.Type then
            ...
            if this.ServiceEndpoints.IsEmpty then
                ...
                |> List.map (fun (Network.EndpointServiceType (serviceEndpoint), locations) ->
                    {| service = serviceEndpoint
                       locations =
                        locations
                        |> List.map (fun location -> location.ArmValue) |})
                               ...
                               properties =
                                ...
                                {| privateIpAllocationMethod = allocationMethod
                                   ...
                                   publicIPAddress =
                                    {| id =
                                        publicIPAddresses
                                            .resourceId(ipConfig.PublicIpName)
                                            .Eval() |}
                                   subnet =
                                    {| id =
                                        subnets
...
type Connection =
    ...
    member private this.VNetGateway2ResourceId =
        ...
        |> Option.map virtualNetworkGateways.resourceId

    member private this.LocalNetworkGatewayResourceId =
        this.LocalNetworkGateway
        |> Option.map localNetworkGateways.resourceId
                            ...
                            {| name = $"eth{index}"
                               properties =
                                {| ipConfigurations =
                                    ...
                                    |> List.mapi (fun index ipConfig ->
                                        {| name =
                                            (ipConfig.Name.IfEmpty $"ipconfig{index + 1}")
                                                .Value
                                           properties =
                                            {| subnet =
                                                {| id =
                                                    { subnets.resourceId (this.VirtualNetwork.Name, ipConfig.SubnetName) with
...
type NetworkPeering =
    ...
    member this.Name =
        this.OwningVNet.Name
        / $"peering-%s{this.RemoteVNet.Name.Value}"
