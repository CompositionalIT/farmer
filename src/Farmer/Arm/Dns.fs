                    ...
                    properties =
                        [ (ttlKey this.ZoneType), box this.TTL
                          ...
                          | NS (NsRecords.Records records) ->
                              "NSRecords",
                              records
                              |> List.map (fun ns -> {| nsdname = ns |})
                              |> box
                          | NS (NsRecords.SourceZone sourceZone) ->
                              "NSRecords",
                              (sourceZoneNSRecordReference sourceZone).Eval()
                              |> box
                          | TXT records ->
                              (txtRecordKey this.ZoneType),
                              records
                              |> List.map (fun txt -> {| value = [ txt ] |})
                              |> box
                          | PTR records ->
                              (ptrRecordKey this.ZoneType),
                              records
                              |> List.map (fun ptr -> {| ptrdname = ptr |})
                              |> box
                          | A (_, records) ->
                              (aRecordKey this.ZoneType),
                              records
                              |> List.map (fun a -> {| ipv4Address = a |})
                              |> box
                          | AAAA (_, records) ->
                              (aaaRecordKey this.ZoneType),
                              records
                              |> List.map (fun aaaa -> {| ipv6Address = aaaa |})
                              |> box
                          | SRV records ->
                              let records =
                                  records
