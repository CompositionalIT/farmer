...
let containerApps =
    ResourceType("Microsoft.App/containerApps", "2022-01-01-preview")

let managedEnvironments =
    ResourceType("Microsoft.App/managedEnvironments", "2022-01-01-preview")
                                               ...
                                               {| ``type`` = "cpu"
                                                  metadata =
                                                   {| ``type`` =
                                                      ...
                                                      value =
                                                       match settings with
                                                       | Utilisation v -> v.Utilisation |> string
                       ...
                       appLogsConfiguration =
                        {| destination = "log-analytics"
                           logAnalyticsConfiguration =
                            {| customerId =
                                LogAnalytics
                                    .getCustomerId(this.LogAnalytics)
                                    .Eval()
                               sharedKey =
                                LogAnalytics
                                    .getPrimarySharedKey(this.LogAnalytics)
                                    .Eval() |} |} |} |}
