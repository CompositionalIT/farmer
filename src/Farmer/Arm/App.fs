...
let daprComponents =
    ResourceType("Microsoft.App/managedEnvironments/daprComponents", "2022-01-01-preview")
    ...
    interface IArmResource with
        ...
        member this.JsonModel =
            {| daprComponents.Create(
                   this.ResourceName,
                   this.Location,
                   [ ResourceId.create (managedEnvironments, this.ManagedEnvironment) ]
               ) with
                properties =
                    {|
                        ...
                        secrets =
                            [
                                for metadata in this.Metadata do
                                    ...
                                    | SecureEnvValue parameter ->
                                        {|
                                            name = metadata.Key.ToLower()
                                            value = parameter.ArmExpression.Eval()
                                        |}
                                    | SecureEnvExpression expr ->
                                        {|
                                            name = metadata.Key.ToLower()
                                            value = expr.Eval()
                                        |}
                            ...
                            [
                                for metadata in this.Metadata do
                                    ...
                                    | SecureEnvExpression _ ->
                                        {|
                                            name = metadata.Key
                                            secretRef = metadata.Key.ToLower()
                                        |}
                                        :> obj
                                    | EnvValue value -> {| name = metadata.Key; value = value |}
                            ]
                    |}
            |}
