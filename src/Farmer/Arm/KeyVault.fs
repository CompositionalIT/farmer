...
type Vault =
    ...
    member private _.ToStringArray s =
        s
        |> Set.map (fun s -> s.ToString().ToLower())
        |> Set.toArray
            ...
            {| vaults.Create(this.Name, this.Location, this.Dependencies, this.Tags) with
                properties =
                    {| tenantId = this.TenantId
                       ...
                       createMode =
                        this.CreateMode
                        |> Option.map (fun m -> m.ToString().ToLower())
                        |> Option.toObj
                    ...
                    {| accessPolicies =
                        [| for policy in this.AccessPolicies do
                               {| objectId = ArmExpression.Eval policy.ObjectId
                                  tenantId =
                                   this.TenantId
                                   |> Option.defaultValue "[subscription().tenantId]"
                                  applicationId =
                                   policy.ApplicationId
                                   |> Option.map string
                                   |> Option.toObj
