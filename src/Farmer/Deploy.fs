...
module Az =
        ...
        let executeAz arguments =
            try
                ...
                let flushContents () =
                    ...
                    [ azProcess.StandardOutput
                      azProcess.StandardError ]
        ...
        let parametersArgument =
            ...
            | parameters ->
                sprintf
                    "--parameters %s"
                    (parameters
                     |> List.map (escapeParameterArguments)
                     |> String.concat " ")
    ...
    let enableStaticWebsite name indexDoc errorDoc =
        [ $"storage blob service-properties update --account-name %s{name} --static-website --index-document %s{indexDoc}"
          yield!
              errorDoc
              |> Option.mapList (sprintf "--404-document %s") ]
        |> String.concat " "
        |> az
...
let listSubscriptions () =
    result {
        ...
        return
            response
            |> Serialization.ofJson<Subscription array>
    }
...
let private prepareForDeployment parameters resourceGroupName (deployment: IDeploymentSource) =
    result {
        ...
        let subscriptionDetails =
            subscriptionDetails
            |> Serialization.ofJson<{| id: Guid; name: string |}>

        printfn "Using subscription '%s' (%O)." subscriptionDetails.name subscriptionDetails.id

        let resourceGroups =
            (resourceGroupName
             :: deployment.Deployment.RequiredResourceGroups)
            |> List.distinct
...
let tryValidate resourceGroupName parameters (deployment: IDeploymentSource) =
    result {
        let! deploymentParameters =
            deployment
            |> prepareForDeployment parameters resourceGroupName
