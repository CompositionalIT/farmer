    ...
    type CorsRule =
        ...
        static member create(?allowedOrigins, ?allowedMethods, ?maxAgeInSeconds, ?exposedHeaders, ?allowedHeaders) =
            ...
            { AllowedOrigins =
              ...
              ExposedHeaders =
                exposedHeaders
                |> mapDefault Specific CorsRule.AllowAll.ExposedHeaders
              AllowedHeaders =
                allowedHeaders
                |> mapDefault Specific CorsRule.AllowAll.AllowedHeaders }
...
module Identity =
    ...
    type ManagedIdentity =
        ...
        static member (+)(a, b) =
            { SystemAssigned =
                (a.SystemAssigned.AsBoolean
                 || b.SystemAssigned.AsBoolean)
                |> FeatureFlag.ofBool
...
module Containers =
    type DockerImage =
        ...
        static member Parse(tag: string) =
            ...
            | [| repo; version |] ->
                match
                    repo.Split([| '/' |], StringSplitOptions.RemoveEmptyEntries)
                    |> List.ofArray
                    with
...
module AvailabilityTest =
    ...
    type TestSiteLocation =
        ...
        static member NorthCentralUS =
            Farmer.Location "us-il-ch1-azr"
            |> AvailabilityTestSite

        static member WestEurope =
            Farmer.Location "emea-nl-ams-azr"
            |> AvailabilityTestSite

        static member SoutheastAsia =
            Farmer.Location "apac-sg-sin-azr"
            |> AvailabilityTestSite

        static member WestUS =
            Farmer.Location "us-ca-sjc-azr"
            |> AvailabilityTestSite

        static member SouthCentralUS =
            Farmer.Location "us-tx-sn1-azr"
            |> AvailabilityTestSite

        static member EastUS =
            Farmer.Location "us-va-ash-azr"
            |> AvailabilityTestSite

        static member EastAsia =
            Farmer.Location "apac-hk-hkn-azr"
            |> AvailabilityTestSite

        static member NorthEurope =
            Farmer.Location "emea-gb-db3-azr"
            |> AvailabilityTestSite

        static member JapanEast =
            Farmer.Location "apac-jp-kaw-edge"
            |> AvailabilityTestSite

        static member AustraliaEast =
            Farmer.Location "emea-au-syd-edge"
            |> AvailabilityTestSite

        static member FranceCentralSouth =
            Farmer.Location "emea-ch-zrh-edge"
            |> AvailabilityTestSite

        static member FranceCentral =
            Farmer.Location "emea-fr-pra-edge"
            |> AvailabilityTestSite

        static member UKSouth =
            Farmer.Location "emea-ru-msa-edge"
            |> AvailabilityTestSite

        static member UKWest =
            Farmer.Location "emea-se-sto-edge"
            |> AvailabilityTestSite

        static member BrazilSouth =
            Farmer.Location "latam-br-gru-edge"
            |> AvailabilityTestSite

        static member CentralUS =
            Farmer.Location "us-fl-mia-edge"
            |> AvailabilityTestSite
...
module private Helpers =
    let (|InBounds|OutOfBounds|) days =
        if days > 365<Days> then
            OutOfBounds days
        elif days < 1<Days> then
            OutOfBounds days
        else
            InBounds days
...
type MetricSetting =
    ...
    static member Create(category, ?retentionPeriod, ?timeGrain) =
        { Category = category
          ...
          RetentionPolicy =
            retentionPeriod
            |> Option.map (fun days -> RetentionPolicy.Create(days, true)) }
