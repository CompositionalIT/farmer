...
| os_disk_caching                        | Sets the host caching mode for the OS disk. Accepted values: `NoCaching`, `ReadOnly`, `ReadWrite`. When not set, no caching property is emitted in the ARM template (Azure applies its own defaults). Does not apply to attached OS disks.                                                                                                                            |
| data_disk_caching                      | Sets the host caching mode for all data disks. Accepted values: `NoCaching`, `ReadOnly`, `ReadWrite`. When not set, no caching property is emitted in the ARM template (Azure applies its own defaults). Does not apply to attached data disks.                                                                                                                       |
